...
export class CPost extends CUqBase {
    ...
    showProductCatalog = async () => {
        ...
        let results = await this.uqs.JkseProduct.GetRootCategory.query({ salesRegion: SALESREGION_CN, language: CHINESE });
    ...
    searchProductCatalogPostCount = async (productcategory: any) => {
        let list = await this.uqs.JkseWebbuilder.SearchProductCategoryPostCount.obj({ productCategory: productcategory });
        return list.postcounts;
    }
    searchProductCatalogChildrenKeys = async (param: any) => {
        let { SALESREGION_CN, CHINESE } = GLOABLE;
        let { productCategory, name } = param
        let results = await this.uqs.JkseProduct.GetChildrenCategory.query({ parent: productCategory.id, salesRegion: SALESREGION_CN, language: CHINESE });
    ...
    showProductCatalogDetil = async (param: any) => {
        let publish = setting.sales.isInner ? 3 : 2;
        this.pageProductCatalogPost = new QueryPager(this.uqs.JkseWebbuilder.SearchProductCategoryPost, 15, 30);
    ...
    showSubject = async (param: any) => {
        let { name, id } = param;
        let pageSubject = new QueryPager(this.uqs.JkseWebbuilder.SearchSubject, 15, 100);
    ...
    showSubjectPost = async (param: any) => {
        let publish = setting.sales.isInner ? 3 : 2;
        this.pageSubjectPost = new QueryPager(this.uqs.JkseWebbuilder.SearchSubjectPost, 15, 30);
    ...
    showDomain = async (param: any) => {
        let { id, name } = param;
        let domain = new QueryPager(this.uqs.JkseCustomer.SearchDomain, 15, 100);
    ...
    showDomainPost_Search = async (param: any, key: any) => {
        let publish = setting.sales.isInner ? 3 : 2;
        this.pageDomainPost = new QueryPager(this.uqs.JkseWebbuilder.SearchDomainPost, 15, 100);
    ...
    searchDomainCount = async (domain: any) => {
        let list = await this.uqs.JkseWebbuilder.SearchDomainPostCount.obj({ domain: domain });
        return list.postcounts;
    }
    ...
    searchCustomerByKey = async (key: string, post: string, domain: any) => {
        this.pageCustomer = new QueryPager(this.uqs.JkseSalesTask.SearchMyCustomerByPost, 15, 30);
        this.pageCustomer.first({ key: key, post: post, domain: domain });
    };

    searchCustomerByCategory = async (key: string, category: any) => {
        this.pageCustomer = new QueryPager(this.uqs.JkseSalesTask.SearchMyCustomerByCategory, 15, 30);
    ...
    showCustomer = async (key: string, param: any) => {
        ...
        let domain = await this.uqs.JkseWebbuilder.PostDomain.query({ post: post.id });
        let catalog = await this.uqs.JkseWebbuilder.PostProductCatalog.query({ post: post.id });
    ...
    addMyCustomerPost = async (post: any, customerid: any) => {
        ...
        await this.uqs.JkseSalesTask.AddMyCustomerPost.submit(param);
        await this.uqs.JkseWebbuilder.TransmitPost.submit({ _post: post });
    };
