...
export class CProduct extends CUqBase {
    ...
    searchByKey = async (par: any) => {
        ...
        this.pageProduct = new QueryPager(this.uqs.JkseProduct.SearchProduct, 15, 30);
    ...
    getInventoryAllocation = async (productId: number, packId: number, salesRegionId: number) => {
        if (this.inventoryAllocationContainer[packId] === undefined)
            this.inventoryAllocationContainer[packId] = await this.uqs.JkseWarehouse.GetInventoryAllocation.table({ product: productId, pack: packId, salesRegion: this.cApp.currentSalesRegion });
    }
    //交货期说明
    getFutureDeliveryTimeDescription = async (productId: number, salesRegionId: number) => {
        if (this.futureDeliveryTimeDescriptionContainer[productId] === undefined) {
            let futureDeliveryTime = await this.uqs.JkseProduct.GetFutureDeliveryTime.table({ product: productId, salesRegion: salesRegionId });
    ...
    searchPromotion = async (key: any, promotionId: any) => {
        ...
        this.promotionPager = new QueryPager(this.uqs.JksePromotion.SearchPromotion, 15, 30);
    ...
    getChemicalInfo = async (productId: number) => {
        if (this.chemicalInfoContainer[productId] === undefined) {
            this.chemicalInfoContainer[productId] = await this.uqs.JkseProduct.ProductChemical.obj({ product: productId });
        }
    }
